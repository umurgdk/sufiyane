...
function SetSeminarDays(seminar, days) {
	if(days["pazartesi"]) {
...
// }}}
// Admin -- Seminerler Yeni (GET)
app.get('/yonetim/seminerler/yeni', function (req, res, next) {
	res.render('seminarNew', {
		title   : "Seminerler - Yeni",
		seminar : null,
		days    : [],
		scripts : [
			'mootools',
...
app.post('/yonetim/seminerler/yeni', function   (req,res,next) {
	var fields = {
		days : req.body["days"],
		name : req.body["name"],
		text : req.body["text"],
	...
	if(fields.days) {
		seminar = SetSeminarDays(seminar, fields.days);
	}
	seminar.name = fields.name || "";
	seminar.text = fields.text || "";
	seminar.active = fields.active || false;
	seminar.shortText = fields.shortText || "";
...
app.get('/yonetim/seminerler/:id/sil', function (req, res, next) {
	var id = req.params.id;
	as.waterfall([
		function (cb) {
...
app.post('/yonetim/seminerler/:id/duzenle', function (req,res,next) {
	...
	function (cb) {
		...
		if (days) {
			seminar = SetSeminarDays(seminar, days);
		}
